...
class DriverStateTracker:
    ...
    def judge_state(self, ear, mar, adjusted_yaw, adjusted_pitch, ear_threshold):
        ...
        # 1. 졸음 판정 (EAR)
        if ear_threshold is not None and ear < ear_threshold:
            if self.sleep_start_time is None:
                self.sleep_start_time = current_time
            if current_time - self.sleep_start_time >= SLEEP_TIME:
                sleep_detected = True
        else:
            self.sleep_start_time = None
        ...
        if mar > MAR_THRESHOLD:
            ...
            if current_time - self.yawn_start_time >= YAWN_TIME:
                yawn_detected = True
        else:
            self.yawn_start_time = None

        # 3. 시선 이탈 판정 (Head Pose Orientation)
        if abs(adjusted_yaw) > YAW_THRESHOLD or abs(adjusted_pitch) > PITCH_THRESHOLD:
            if self.distraction_start_time is None:
                self.distraction_start_time = current_time
            if current_time - self.distraction_start_time >= DISTRACTION_TIME:
    ...
    while True:
        ...
        if result.multi_face_landmarks:
            ...
            if ear_threshold is None:
                ...
                state = tracker.judge_state(ear, mar, adjusted_yaw, adjusted_pitch, ear_threshold)
